---
subtitle: "2027948 -- RADAR AND LIDAR REMOTE SENSING"
title: "<font style='font-size:1em;'>🗓️ Week 02<br/> SAR Basics</font>"
author: Ivan Lizarazo
institute: '[Universidad Nacional de Colombia](#)'
date: 3 September 2026
date-meta: 3 September 2026
date-format: "DD MMM YYYY"
toc: true
toc-depth: 1
toc-title: "What we will cover today:"
center-title-slide: false
from: markdown+emoji
format:
  revealjs: 
    fig-responsive: true
    theme: simple
    slide-number: true
    mouse-wheel: false
    preview-links: auto
    logo: /figures/icons/ARS-1.png
    css: /css/styles_slides.css
    footer: '2027449 -- PERCEPCION REMOTA AVANZADA'
---

# Introduction

Checking homework:

1. Reading 

2. Practical 

3. Video 


# What's RADAR?

Based on the principle of *echolocation*

RAdio Detection and Ranging

SAR is not just a radar: it is an imaging radar system,

SAR's antenna moves while emiting signals and receiving  echos

# Radar Operation

## Radar is an echo measurement system

A pulse of duration $\tau$ travels at the speed of light $c$

The two-way range distance between antena and target is the velocity times the time delay $\Delta T$

Thus, the range distance to the target is $R = \frac {c\Delta T}{2}$

## Measuring the backscatter

![](figures/measuring.png)

## Radar collection angles

![](figures/collection.png)

## Range and azimuth dimension

![](figures/ra_dimension.png)

## Side-looking geometry 

![](figures/pyramids.png)

# [The Radar Equation](https://www.radartutorial.eu/01.basics/The%20Radar%20Range%20Equation.en.html)

![](figures/radar_equation.png)

# Frequency and Wavelength

## The wavelengths
![](figures/radar_longs.png)

## The benefits

![](figures/benefits.png)

## Weather independence

Sentinel-1 | Sentinel-2
- | - 
![alt](figures/sentinel1.gif) | ![alt](figures/sentinel2.gif)

## Radar bands

![](figures/radar_bands.png)

## Radar signal penetration

![](figures/penetration.png)

# Polarization and scattering

## Electromagnetic waves

![](figures/em_waves.png)

## Polarization

![](figures/polarization.png)

## SAR polarization

![](figures/sar_polarization.png)

## Backscattering

![](figures/scatterers.png)

## Relative scattering

![](figures/relative.png)

## SAR signatures

![](figures/pol_signatures.png)

# Spatial resolution

## SAR system

![](figures/sar_system.png)

## Range resolution

![](figures/range_res.png)


## Two distinct reflections

![](figures/Two_houses.png)



## Real vs Synthetic Aperture


![](figures/SyntethicRadar.png)



## Azimuth resolution

![](figures/azi_res.png)

## A Visual intro to SAR

[Intro2radar](https://www.geos.ed.ac.uk/~ihw/hype/radar/intro2radar.html) by Iain Woodhouse

## Speckle

![](figures/speckle.png)

## Geometric distorsions

![](figures/geometric.png)

## Backscatter variables

![](figures/table_variables.png)

## Backscatter reference areas

![](figures/back_variables.png)

# To do before next class 

## Read this document

[A layman interpretation guide to L-band and C-band SAR data](https://ceos.org/ard/files/Laymans_SAR_Interpretation_Guide_2.0.pdf) 

CEOS (2018) 

## Watch this video

[See the world through SAR](https://www.iceye.com/vos-talk2-video-page) 

Tom Ager - ICEYE -  2023

## Next practical exercise

Crop monitoring with Sentinel-1 data